...
if [ $stage -le 1 ]; then
  ...
  for name in sre swbd sre16_eval_enroll sre16_eval_test sre16_major; do
    ...
    local/xvector/compute_vad_decision.sh --nj 40 --cmd "$train_cmd" \
...
if [ $stage -le 3 ]; then
  ...
  local/xvector/prepare_feats_for_egs.sh --nj 40 --cmd "$train_cmd" \
...
if [ $stage -le 7 ]; then
  ...
  # score normalization.
  local/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 6G" --nj 40 \
  ...
  local/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 12G" --nj 40 \
